-- ============================================================
-- BBR Fragance - Migracion: Comprobantes Fiscales (NCF)
...
SET @col_exists = (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = 'bbr_fragance' AND TABLE_NAME = 'customers' AND COLUMN_NAME = 'rnc');
SET @sql = IF(@col_exists = 0, 'ALTER TABLE customers ADD COLUMN rnc VARCHAR(11) NULL AFTER name', 'SELECT 1');
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;

SET @col_exists = (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = 'bbr_fragance' AND TABLE_NAME = 'customers' AND COLUMN_NAME = 'cedula');
SET @sql = IF(@col_exists = 0, 'ALTER TABLE customers ADD COLUMN cedula VARCHAR(13) NULL AFTER rnc', 'SELECT 1');
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;

SET @idx_exists = (SELECT COUNT(*) FROM INFORMATION_SCHEMA.STATISTICS WHERE TABLE_SCHEMA = 'bbr_fragance' AND TABLE_NAME = 'customers' AND INDEX_NAME = 'idx_rnc');
SET @sql = IF(@idx_exists = 0, 'ALTER TABLE customers ADD INDEX idx_rnc (rnc)', 'SELECT 1');
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;

-- 3. Agregar campos NCF a ventas
SET @col_exists = (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = 'bbr_fragance' AND TABLE_NAME = 'sales' AND COLUMN_NAME = 'ncf_number');
SET @sql = IF(@col_exists = 0, 'ALTER TABLE sales ADD COLUMN ncf_number VARCHAR(13) NULL AFTER sale_number', 'SELECT 1');
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;

SET @col_exists = (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = 'bbr_fragance' AND TABLE_NAME = 'sales' AND COLUMN_NAME = 'ncf_type');
SET @sql = IF(@col_exists = 0, 'ALTER TABLE sales ADD COLUMN ncf_type VARCHAR(3) NULL AFTER ncf_number', 'SELECT 1');
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;

SET @col_exists = (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = 'bbr_fragance' AND TABLE_NAME = 'sales' AND COLUMN_NAME = 'customer_rnc');
SET @sql = IF(@col_exists = 0, 'ALTER TABLE sales ADD COLUMN customer_rnc VARCHAR(11) NULL AFTER ncf_type', 'SELECT 1');
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;

SET @idx_exists = (SELECT COUNT(*) FROM INFORMATION_SCHEMA.STATISTICS WHERE TABLE_SCHEMA = 'bbr_fragance' AND TABLE_NAME = 'sales' AND INDEX_NAME = 'idx_ncf');
